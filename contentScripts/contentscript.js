...
(function(regexHelpers, undefined){
    regexHelpers.usdFormat = function(string){
        return string.match(/(\$|dollar|USD)+\s*\d/);
    };
    ...
    regexHelpers.pundFormat = function(string){
        return string.match(/(£|pund|GBP)+\s*\d/);
    };
...
(function(currencyIntergrator, undefined){
    ...
    var polling = 0;
    var selectionText = '';
    currencyIntergrator.clickOption = 0;
    ...
    currencyIntergrator.onMouseUp = function(event){
        ...
        if(internalEvent.button === currencyIntergrator.clickOption) {
...
chrome.storage.sync.get('click_option', function(items){
    if(items.click_option){
      currencyIntergrator.clickOption = items.click_option;
      //console.log("sync get");
      //console.log(currencyIntergrator.clickOption);
    }
});
...
chrome.storage.onChanged.addListener(function(changes, namespace){
  for (key in changes) {
    ...
    currencyIntergrator.clickOption = storageChange.newValue;
  }
});
